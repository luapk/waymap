...
// Highly stable, pre-cleaned WGS84 London Boroughs dataset
const GEO_URL = "https://skgrange.github.io/www/data/london_boroughs.json";
...
const COHORTS = {
  ...
  "The Last Mile Londoner": { color: "#B8D940", stroke: "#C7E360", boroughs: ["Barking and Dagenham","Newham","Lewisham","Croydon"], need: "First/last mile to rail", level: 4 },
};
...
export default function WaymoMap() {
  ...
  // 1. Initial Sizing for 4K / Responsive
  useEffect(() => {
  ...
  // 3. One-Time Projection Calculation (No lag on re-renders)
  const { pathGenerator, mapW, mapH, ox, oy } = useMemo(() => {
    if (!geoData || dims.w === 0) return { pathGenerator: null };

    // Define the exact drawing box for the map (leaves room for side panel)
    const mapWidth = dims.w * 0.70;
    const mapHeight = dims.h * 0.85;
    const offsetX = dims.w * 0.02;
    const offsetY = dims.h * 0.08;

    // mathematically forces ALL boroughs to fit exactly in the box
    const projection = d3.geoMercator().fitExtent(
      [[20, 20], [mapWidth - 20, mapHeight - 20]], 
      geoData
    );
  ...
  if (status === "loading" || dims.w === 0) {
    return <div style={{ background: BG, color: "#00E5A0", height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "monospace", letterSpacing: 2 }}>LOADING PRECISION GEODATA...</div>;
  }
  if (status === "error") {
    return <div style={{ background: BG, color: "#EF4444", height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "monospace", letterSpacing: 2 }}>DATA CONNECTION FAILED.</div>;
  }

  // Handle Ultra-Fast Hover Interaction (Bypasses React DOM diffing)
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", background: BG, overflow: "hidden", position: "relative", fontFamily: "monospace" }}>
      ...
      {/* HIGH-PERFORMANCE CSS HOVER RULES */}
      <style>{`
        .london-borough {
        ...
        .cohort-active { opacity: 0.85; }
        .cohort-inactive { opacity: 0.25; }
      `}</style>
      ...
      >
        <g transform={`translate(${ox}, ${oy})`}>
          ...
          {/* Render All 33 Boroughs */}
          <g>
            {geoData.features.map((feature, i) => {
              const name = feature.properties.name || `Borough-${i}`;
              const cohort = getCohort(name);
              const pathString = pathGenerator(feature);
              
              if (!pathString) return null; // Safe fallback
              ...
              return (
                <path
                  ...
                  className={`london-borough ${cohort ? 'cohort-active' : 'cohort-inactive'}`}
                  // Data attributes for the fast tooltip
                  data-name={name}
                  data-cohort={cohort ? cohort.cohortName : "Non-Pilot Area"}
      ...
      {/* COHORT KEY SIDE PANEL */}
      <div style={{position:"absolute", right:0, top:0, width:"25%", height:"100%", background:"rgba(6, 11, 20, 0.9)", borderLeft:"1px solid #1A3050", padding: "50px 40px", minWidth: 320, zIndex: 10}}>
